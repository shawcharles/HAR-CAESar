...
def test_caesar_run():
    ...
    y = returns['^GSPC'].values # Select one asset
    ...
    print(f"Running CAESar on SP500 data...")
    ...
    plt.title(f'CAESar Forecasts (SP500 Data, theta={theta})')
    plt.legend()
    ...
    # Assertions
    assert len(q_pred) == test_size
    assert len(e_pred) == test_size
    # Sanity check on violation rate (should be somewhat close to theta)
    # But this is a stochastic model and short sample, so loose bounds
    assert 0.0 < violation_rate < 0.1
